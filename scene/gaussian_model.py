...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        # plane function
        self.plane_normal = torch.empty(0)
        self.plane_depth = torch.empty(0)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self.plane_normal = torch.nn.Parameter(torch.zeros(3).to(self._xyz.device).requires_grad_(True)) 
        self.plane_depth = torch.nn.Parameter(torch.tensor(0.4).to(self._xyz.device).requires_grad_(True)) 
    ...
    def save_ply(self, path, path2):
        mkdir_p(os.path.dirname(path))

        xyz = self._xyz.detach().cpu().numpy()
        # normals = self._normal.detach().cpu().numpy()
        # normals2 = self._normal2.detach().cpu().numpy()
        # bug here ! need to fix, now normal won't be stored
        normals =  np.zeros_like(xyz)
        normals2 = np.zeros_like(xyz)
        f_dc = self._features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        plane_normal = self.plane_normal.detach()
        plane_depth = self.plane_depth.detach()
        torch.save(plane_normal,os.path.join(path2, "plane_normal.pth"))
        torch.save(plane_depth,os.path.join(path2, "plane_depth.pth"))
    ...
    def load_ply(self, path, path2, path3):
        ...
        normal = np.stack((np.asarray(plydata.elements[0]["x"]),
                            np.asarray(plydata.elements[0]["y"]),
                            np.asarray(plydata.elements[0]["z"])),  axis=1)
        normal2 = np.stack((np.asarray(plydata.elements[0]["x"]),
                            np.asarray(plydata.elements[0]["y"]),
                            np.asarray(plydata.elements[0]["z"])),  axis=1)
        ...
        self.plane_normal = nn.Parameter(torch.load(path2,map_location=torch.device("cuda")).requires_grad_(True))
        self.plane_depth = nn.Parameter(torch.load(path3,map_location=torch.device("cuda")).requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_normal, new_normal2):
        d = {"xyz": new_xyz,
